...
import { getAllServices } from '@/core/infrastructure/services/tab-agent.service'
...
export default function ServicesTab({updateStepFour}) {
  ...
  return (
    <form className="form-style1" id='property-tab-four'>
              ...
              services && services.map((el) => (
                <div className="col-2 mb-2" key={el.value}>
                  <div className='check-item' key={el.value} >
                    ...
                    />
                    <label key={el.label} htmlFor={`ckeck-service-${el.value}`} >{el.label}</label>
                  </div>
                </div>
              ))
            }
                ...
                <label className='title-add mb-1' key={add.id}>{add.title}</label>
                {
                  add.services && add.services.map((service) => (
                    <div className='check-item mb-2' key={service.id}>
                      ...
                      />
                      <label key={service.label} htmlFor={`ckeck-service-${service.value}`} >{service.label}</label>
                  </div>
                  ))
                }
              </div>
